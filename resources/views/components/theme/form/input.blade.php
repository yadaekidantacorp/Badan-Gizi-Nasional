...
@if($inputGroup == false)
    ...
    @endif
    <input {{ $attributes->whereStartsWith('wire:model') }} id="{{ $id ?? '' }}" type="{{ $type ?? 'text' }}" placeholder="{{ $placeholder }}" autocomplete="{{ $autocomplete ?? 'off' }}" class="{{ $inputStyle }} @if($this->$wireModel != null) @error($wireModel) is-invalid @else is-valid @enderror @endif" {{ $focus == true ? 'autofocus' : '' }} />
    @error($wireModel)
