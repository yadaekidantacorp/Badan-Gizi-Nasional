...
$save = function () {
    ...
    $this->dispatch('toast-success', message: 'Pengguna berhasil disimpan', title: config('app.name'));
    return $this->redirect(route('app.ministry.index'), navigate: true);
};
?>
<title>{{config('app.name')}}: Tambah Pengguna</title>
<x-app-layout>
        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex align-items-stretch">
            <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                ...
                <div class="d-flex align-items-center gap-2 gap-lg-3">
                    <a href="{{ route('app.user.index') }}" wire:navigate class="btn btn-flex btn-outline btn-color-gray-700 btn-active-color-primary bg-body h-40px fs-7 fw-bold">Kembali</a>
                </div>
            </div>
        </div>
    </div>
