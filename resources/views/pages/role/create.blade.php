...
$save = function () {
    ...
    Ministry::castAndCreate([
        'name' => $this->nama,
        'slug' => Str::slug($this->nama),
        'thumbnail' => $thumbnail ?? null
    ]);
    $this->dispatch('toast-success', message: 'Peran Pengguna berhasil disimpan', title: config('app.name'));
    return $this->redirect(route('app.ministry.index'), navigate: true);
};
?>
<title>{{config('app.name')}}: Tambah Peran Pengguna</title>
<x-app-layout>
        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex align-items-stretch">
            <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                ...
                <div class="d-flex align-items-center gap-2 gap-lg-3">
                    <a href="{{ route('app.role.index') }}" wire:navigate class="btn btn-flex btn-outline btn-color-gray-700 btn-active-color-primary bg-body h-40px fs-7 fw-bold">Kembali</a>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card">
                <div class="card-header border-0 pt-6">
                    <div class="card-title">
                        <h3 class="card-label">
                            Form Tambah Data Peran Pengguna
                        </h3>
                    </div>
                </div>
                <div class="card-body">
                    <x-form.form function="save">
                        @include('pages.role.form')
