...
middleware(['auth']);
name('app.position.index');

usesPagination(theme: 'bootstrap');
state(['search','layout','order'])->url();
$collection=computed(function(){
    $position = Position::where('name', 'like', '%'.$this->search.'%');
    if($this->order == 2){
        $position->orderBy('name', 'desc');
    }elseif($this->order == 1){
...
?>
<title>{{config('app.name')}}: Posisi</title>
<x-app-layout>
    <div id="kt_app_toolbar" class="app-toolbar pt-2 pt-lg-10">
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex align-items-stretch">
            <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                <div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
                    <h1 class="page-heading d-flex flex-column justify-content-center text-gray-900 fw-bold fs-3 m-0">Posisi</h1>
                ...
                <div class="d-flex align-items-center gap-2 gap-lg-3">
                    <a href="{{ route('app.position.create') }}" wire:navigate class="btn btn-flex btn-outline btn-color-gray-700 btn-active-color-primary bg-body h-40px fs-7 fw-bold">Tambah Posisi</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <div id="kt_app_content_container" class="app-container container-fluid">
            <form>
                <div class="card mb-7">
                    ...
                    <div class="card-body">
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="position_table">
                            <thead>
                                <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                                    <th class="w-10px pe-2">
                                        <div class="form-check form-check-sm form-check-custom form-check-solid me-3">
                                            <input class="form-check-input" type="checkbox" data-kt-check="true" data-kt-check-target="#position_table .form-check-input" value="1" />
                                        </div>
                                    </th>
